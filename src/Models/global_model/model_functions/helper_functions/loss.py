...
def individual_loss(mask, p_matrix, n_holdout):
    """
    ...
    def loss(y_true, y_pred):
        ...
        # tf.print("individual_loss debug: y_true.shape", tf.shape(y_true), " y_pred.shape", tf.shape(y_pred))
        # tf.print("individual_loss debug: mask.shape", tf.shape(mask), " time_len", time_len)
        # tf.print("individual_loss debug: n_obs", n_obs, " n_holdout", n_holdout, " time_len", time_len)
        ...
        # n_obs = tf.cast(tf.shape(y_true_transf)[1], dtype=tf.float32)
        ...
        if n_obs == n_holdout:
            ...
            P_dyn = tf.cast(p_matrix[-n_holdout:, -n_holdout:], dtype=tf.float32)
        else:
            mask_dyn = mask[:, :time_len, :]    
            ...
            # tf.print(">>> individual_loss debug: y_pred_transf.shape:", tf.shape(y_pred_transf))
            
            
            #count number of non-missing observations from y_true_transf
    
            P_dyn = tf.cast(p_matrix[:n_obs, :n_obs], dtype=tf.float32)
