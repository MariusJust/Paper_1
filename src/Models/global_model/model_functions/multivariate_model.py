...
from .helper_functions import initialize_parameters, Preprocess, individual_loss, HoldoutMonitor, WithinHelper, HoldoutLoss
...
class MultivariateModel:
    ...
    def __init__(self, node, x_train, y_train, dropout, penalty, country_trends, dynamic_model, within_transform, x_train_val=None, y_train_val=None, y_val=None, x_val=None, holdout=0):
        """
        Instantiating class.
    ...
    def fit(self, lr, min_delta, patience, verbose):
        ...
        if self.holdout>0 and self.within_transform==True:
            ...
            p_tensor=tf.convert_to_tensor(P_matrix_full, dtype=tf.float64)
            
            
            y_true_target_train=tf.matmul(p_tensor, tf.reshape(self.targets[~self.Mask], (1, -1, 1)))[:, :self.noObs['train'], :]
            
           
            # y_true_target_train=tf.matmul(p_tensor, tf.reshape(self.targets[~self.Mask], (1, -1, 1)))[:, :-self.holdout*self.N['global'], :]
            y_true_target_val=tf.matmul(p_tensor, tf.reshape(self.targets[~self.Mask], (1, -1, 1)))[:, self.noObs['train']:, :]
