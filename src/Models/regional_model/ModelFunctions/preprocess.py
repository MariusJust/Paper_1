...
def Preprocess(self):
    ...
    compute_mask(self, self.x_train['temp'])
...
def compute_x_train(self, x_train):
    ...
    if self.formulation == 'regional':
        ...
        for key, var in x_train.items():
            self.x_train_transf[key]['global'] = var['global']
            self.x_train_transf[key]['global'] =np.array(x_train[key]['global'].copy())
        
            global_stats = compute_stats(self.x_train_transf[key]['global'].values)
...
def compute_y_train(self, y_train):
    ...
    if self.formulation == 'regional':
            ...
            self.y_train_transf[key]['global'] = np.array(y_train[key]['global'].copy())
...
def compute_mask(self, x_train):
    if self.formulation == 'regional':
        ...
        self.mask['global'] = np.isnan(x_train['global'])
        
        # also calculate the number of non-NaN values
        non_na = np.sum(~np.isnan(x_train['global']), axis=1) > 0
        self.time_periods_not_na['global'] = non_na
        self.time_periods_na['global'] = np.sum(~non_na)
        self.noObs['global'] = self.N['global']* self.T - np.isnan(self.x_train_transf_temp['global']).sum()
