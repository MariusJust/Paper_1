...
class MainLoop:
    def __init__(self, master, node):
        ...
        # Load data
        if self.master.data is not None: #ie we are running a Monte Carlo experiment
            from simulations.simulation_functions import Pivot
            self.growth, self.precip, self.temp = Pivot(self.master.data)
        else:   
            self.growth, self.precip, self.temp = load_data('IC', self.master.n_countries, self.master.time_periods)
