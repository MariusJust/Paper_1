...
################################### Part 1 #######################################################
#                                                                                                #                
#                         Compare baseline regression with the new data                          #
#                                                                                                #
##################################################################################################

# Load the updated dataset
Updated_Data <- read_xlsx("Replication/Updated_Data/MainData.xlsx")
# Define the temperature variable
Updated_Data$temp <- Updated_Data$Temp_PopWeight
# Define the quadratic term for temperature
Updated_Data$temp_sq <- Updated_Data$temp^2
Updated_Data$precip<-Updated_Data$Precip_PopWeight
Updated_Data$precip_sq<-Updated_Data$precip^2
...
model_new <- lm( growthWDI~ temp + temp_sq + precip + precip_sq + factor(Year) + factor(ISO), data = Updated_Data)
...
# Save the regression table to a LaTeX file
stargazer(model_new, model,
          title = "Comparison of Models (Specific Coefficients)",
          column.labels = c("Updated Data", "Original Data"),
