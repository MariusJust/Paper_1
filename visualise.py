...
# model_instance = Model(node, x_train, y_train=growth, dropout=dropout)

# # Access the underlying Keras model (created in _model_definition)
# model = model_instance.model

# # --- Prepare to log the graph ---
# # Create a log directory for TensorBoard
# logdir = "logs/graph"
# writer = tf.summary.create_file_writer(logdir)

# concrete_func = tf.function(model).get_concrete_function(model.inputs_temp, model.inputs_precip)
# # Get a concrete function from the model. This "freezes" the graph.

# # Log the graph to the writer (this happens just once)
# with writer.as_default():
#     tf.summary.graph(concrete_func.graph, step=0)
#     writer.flush()
...
##############################################  plotting 3D  ########################################################
results=dict(np.load('results/09042025/results.npy', allow_pickle=True).item())

#find the best 10 models 
min_nodes = sorted(results, key=lambda node: results[node])[:10]
...
no_inits = 10                   # Number of different initializations per node
...
model=Model(min_node, x_train=x_train, y_train=growth, dropout=dropout)

model.model.fit(lr=lr, min_delta=min_delta, patience=patience, verbose=verbose)

model.save_params()

model.model.summary()
#loadig the best model weights for reproducability
model.load_params('Model Parameters/AIC/03042025/' +  str(min_node)+'.weights.h5')
...
# wandb.init(tensorboard=True, project="test")

# tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.INFO)


# model1=Model((4,), x_train=x_train, y_train=growth, dropout=0.2)


# model1.load_params('Model Parameters/BIC/01042025/' +  str((4,))+'.weights.h5')
# model1.fit(lr=lr, min_delta=min_delta, patience=patience, verbose=verbose)
...
# model1.in_sample_predictions()
# model2=Model((4,), x_train=x_train, y_train=growth, dropout=0)
# model2.load_params('Model Parameters/BIC/01042025/' +  str((4,))+'.weights.h5')
# model2.fit(lr=lr, min_delta=min_delta, patience=patience, verbose=verbose, log_dir=log_dir)
...
# # Create a surface plot
# fig = go.Figure(data=[go.Surface(z=Growth1, x=T, y=P, colorscale='Viridis', opacity=0.8, name='Model with Dropout'),
#                       go.Surface(z=Growth2, x=T, y=P, colorscale='Cividis', opacity=0.8, name='Model without Dropout')])
# # Update layout with labels, title, and other settings
# fig.update_layout(
#     title='3D Surface Plot: Growth vs. Temp & Precip (Model 4,)',
#     scene=dict(
...
# # Show the plot
# fig.show()
# pio.write_html(fig, file='images/interactive_3d_plot(4,)_dropout_comparison.html', auto_open=False)
