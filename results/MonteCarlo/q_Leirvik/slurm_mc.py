...
def job_func(job_name):
    ...
    s += 'python -u src/simulations/monte_carlo.py\
    > >(tee slurm-${SLURM_JOB_ID}.out) \
    2> >(grep -Ev --line-buffered \
         "All log messages before absl::InitializeLog|Unable to register cuDNN factory|Unable to register cuBLAS factory" \
         >&2)\n'
    return s
