...
source("DataGeneration/helper_functions.R")


#Retrieve longitude and latitudes 
unpack(dim(), c("lon", "lat"))

countries <-unique(read_excel("DataGeneration/Data/MainData.1.xlsx", range=cell_cols("B:C"))) 


#Retrieve the coordinates that define the borders for each country
matched_country_shp <- read_sf("DataGeneration/Data/national-identifier-grid/gpw_v4_national_identifier_grid_rev11_30_min.shp") %>%
...
nc_file_density <- nc_open("DataGeneration/Data/gpw_v4_population_density_rev11_30_min.nc")
...
nc_file_precip <- nc_open("DataGeneration/Data/cru_ts4.08.1901.2023.pre.dat.nc")
nc_file_temp <- nc_open("DataGeneration/Data/cru_ts4.08.1901.2023.tmp.dat.nc")
...
total_densities <- read_csv("DataGeneration/TotalDensities.csv")
...
#Writing out the final data
write.csv2(out_df, "DataGeneration/final_output.csv")
